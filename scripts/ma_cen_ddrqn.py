...
from rlmamr.MA_cen_ddrqn.team_cen import Team_RNN
from rlmamr.MA_cen_ddrqn.utils.memory_cen import ReplayMemory_rand, ReplayMemory_epi
from rlmamr.MA_cen_ddrqn.utils.utils import save_check_point
from rlmamr.MA_cen_ddrqn.learning_methods_cen import QLearn_squ_cen_0
...
def train(env_name, grid_dim, obs_one_hot, target_flick_prob, agent_trans_noise, env_terminate_step, 
          ...
          trace_len, sub_trace_len, batch_size,sort_traj, rnn, rnn_input_dim, rnn_layer_num, rnn_h_size, run_id, 
          resume, seed, save_dir, device, **kwargs):
    ...
    if sample_epi:
        ...
        if cen_mode == 2:
            memory = ReplayMemory_rand_with_filter(env.n_agent, env.obs_size, trace_len, batch_size, size=replay_buffer_size)
        else:
