...
from rlmamr.MA_hddrqn.team import Team_RNN
from rlmamr.MA_hddrqn.utils.memory import ReplayMemory_rand, ReplayMemory_epi
from rlmamr.MA_hddrqn.utils.utils import save_check_point
...
def train(env_name, grid_dim, obs_one_hot, target_flick_prob, agent_trans_noise, env_terminate_step, n_env, n_agent, 
          total_epi, replay_buffer_size, sample_epi, dynamic_h, init_h, end_h, h_stable_at, cen_train, eps_l_d, 
          eps_l_d_steps, eps_e_d, h_explore, db_step, optim, l_rate, discount, huber_l, g_clip, g_clip_v, g_clip_norm, 
          g_clip_max_norm, start_train, train_freq, target_update_freq, trace_len, sub_trace_len, batch_size,
          sort_traj, rnn, rnn_input_dim, rnn_layer_num, rnn_h_size, run_id, resume, seed, save_dir, device, **kwargs):
    ...
    hyper_params = {'centralized_training': cen_train,
...
def main():
    ...
    parser.add_argument('--init_h', action='store', type=float, default=0.2)
    parser.add_argument('--end_h', action='store', type=float, default=0.4)
    ...
    parser.add_argument('--cen_train', action='store_true')
